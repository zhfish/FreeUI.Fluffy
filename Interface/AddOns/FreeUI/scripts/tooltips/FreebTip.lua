	...
	if (UnitExists(unit)) then
		...
		if (isPlayer) then
			...
			if (guild and strlen(guild) > 0) then
				isInGuild = true
				if (cfg.showGRank == 0) then gRank = nil end
		...
		if (level) then
			...
			if (levelLine) then
				...
				if (cfg.showReaction ~= 0 and not (isPlayer or isBattlePet)) then
					...
					if (reaction) then
						lvltxt = levelLine:GetText()
						levelLine:SetFormattedText("%s "..hex(unitColor(unit)).."%s|r", lvltxt, reaction)
					end
				end
			end
		end
...
local function gtSBValChange(self, value)
	...
	if (UnitExists(unit)) then
		self:SetStatusBarColor(unitColor(unit))
