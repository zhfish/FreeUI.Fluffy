...
local function updateHotkey(self)
	...
	if showHotKey then
		if not self.styledHotkey then
			ho:ClearAllPoints()
			ho:SetWidth(0)
			ho:SetPoint("BOTTOMRIGHT", -1, 1)
		--	F.SetFS(ho)
			ho:SetFont("Interface\\AddOns\\FreeUI\\media\\supereffective.ttf", 8, "OUTLINEMONOCHROME")
...
--	F.SetFS(co)
	co:SetFont("Interface\\AddOns\\FreeUI\\media\\supereffective.ttf", 8, "OUTLINEMONOCHROME")
	co:ClearAllPoints()
	co:SetPoint("BOTTOMLEFT", 2, 2)
