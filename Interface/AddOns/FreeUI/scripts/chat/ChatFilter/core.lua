...
local AddonLoading, FriendsLoading, prevLineId, prevMsg, adding, addedtime, addedplayer, Latency
...
local function ChatFrames_OnUpdate(self, elapsed)
	...
	for id, timeout in pairs(resetTime) do
		if (craftFinished and timeout <= GetTime()) then
...
if (Config.FilterByLevel) then
	hooksecurefunc("ToggleFriendsFrame", function()
		local ChatFrameff, Time = CreateFrame("Frame"), GetTime()
		ChatFrameff:SetScript("OnUpdate", function(self, elapsed)
			FriendsLoading = true
...
ChatFilter:SetScript("OnEvent", function(self, event)
	...
	elseif (event == "TRADE_SKILL_CLOSE") then
		craftCount = 0
		craftFinishing = true
		ChatFilter:UnregisterEvent("UNIT_SPELLCAST_START")
	elseif (event == "UNIT_SPELLCAST_START") then
		...
		if (isTradeSkill) then
			...
			ChatFilter:RegisterEvent("UNIT_SPELLCAST_INTERRUPTED")
		end
	elseif (event == "UNIT_SPELLCAST_INTERRUPTED") then
		craftCount = 0
		craftFinishing = true
		ChatFilter:UnregisterEvent("UNIT_SPELLCAST_INTERRUPTED")
	end
end)
...
local function ChatFilter_Rubbish(self, event, msg, player, _, _, _, flag, _, _, channel, _, lineId, guid)
	...
	if (not Config.ScanOurself) then
		...
		if (gsub(player, "-", "") == fullName..realm) then return end
	end
	if (lineId ~= prevLineId) then
		local player, Time, Name, Server = player, GetTime()
		...
		if (event == "CHAT_MSG_WHISPER") then
			...
			for i = 1, select(2, BNGetNumFriends()) do
				...
				for j = 1, toon do
					local _, rName, rGame, rRealm = BNGetFriendGameAccountInfo(i, j)
					if (rName and rRealm and strlen(rRealm) > 0 and rRealm ~= GetRealmName()) then
						rName = rName.."-"..rRealm
					end
		...
		for i = 1, getn(Config.WhiteList) do
			if (msg:find(Config.WhiteList[i]) or prevMsg:find(Config.WhiteList[i])) then
				return
			end
		end
		for i = 1, getn(Config.BlackList) do
			if (msg:find(Config.BlackList[i]) or prevMsg:find(Config.BlackList[i])) then
				if (Config.Debug) then
		...
		if (Config.FilterByLevel and (event == "CHAT_MSG_WHISPER" or (not Config.OnlyOnWhisper and event ~= "CHAT_MSG_WHISPER"))) then
			...
			if (CanComplainChat(lineId) and not (AddonLoading or FriendsLoading or added or adding or tonumber(player) or UnitIsInMyGuild(player))) then
	...
	if (Config.FilterByLevel) then
		if (AddonLoading) then
			if (msg:find(friendaddedmsg)) then
				local player = gsub(msg, gsub(friendaddedmsg, "%p", ""), "")
				if (not tonumber(player)) then
					AddedTable[player] = true
				end
				return true
			end
...
local function ChatFilter_CreateMSG(self, event, msg)
	...
	if (Config.MergeCraftMSG) then
		...
		if (msg:find(L["You"])) then return end
		for i = 1, getn(lootmsg) do
			if (msg:find(lootmsg[i])) then return end
		end
		if (not prevCraftTime or GetTime() - prevCraftTime > 10) then
			prevCraftTime = GetTime()
			prevCraftMsg = msg
			return
		end
		if (msg == prevCraftMsg) then
			return true
		end
		prevCraftTime = GetTime()
		prevCraftMsg = msg
