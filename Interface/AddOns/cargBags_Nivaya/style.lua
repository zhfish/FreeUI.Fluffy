...
local createIconButton = function (name, parent, texture, point, hint, isBag)
	local font = "Interface\\AddOns\\cargBags_Nivaya\\media\\pixel.ttf"
	local button = CreateFrame("Button", nil, parent)
	button:SetWidth(17)
	button:SetHeight(17)
	...
	button.tooltip:SetFont(font, 8, "OUTLINEMONOCHROME")
...
function MyContainer:OnCreate(name, settings)
	...
	caption:SetFont(font, 8, "OUTLINEMONOCHROME")
	if(caption) then
	...
	if (tBag or tBank or tReagent) then
		...
		fs:SetFont(font, 8, "OUTLINEMONOCHROME")
		fs:SetJustifyH("LEFT")
	...
	if tBag then
		...
		self.hint:SetFont("Interface\\AddOns\\cargBags_Nivaya\\media\\pixel_.ttf", 8, "OUTLINEMONOCHROME")
