...
local font_CN_Size = "8"
local font_EN      = "Fonts\\HOOG0555.ttf"
local font_EN_Size = "8"

local colors = setmetatable({
	power = setmetatable({
		["MANA"] = {104/255, 195/255, 253/255},
		["RAGE"] = {.9, .1, .1},
		["FUEL"] = {0, 0.55, 0.5},
		["FOCUS"] = {.9, .5, .1},
		["ENERGY"] = {.9, .9, .1},
...
local Shared = function(self, unit, isSingle)
	...
	--shift + left click = set/clear focus
    local ModKey = 'Shift'
    local MouseButton = 1
...
--		raidPos = C.unitframes.raid
		raidPos = {"RIGHT", UIParent, "CENTER", -260, -120}
	end

	local focus = spawnHelper(self, 'focus', "TOPLEFT", target, "BOTTOMRIGHT", 100, 120)
	spawnHelper(self, 'focustarget', "LEFT", focus, "RIGHT", 4, 0)
	spawnHelper(self, 'pet', "TOPRIGHT", player, "TOPLEFT", -4, 0)
	...
	for n = 1, MAX_BOSS_FRAMES do
		spawnHelper(self, 'boss' .. n, 'RIGHT', UIParent, 'RIGHT', -120, 180 - (60 * n))
	end

	if C.unitframes.enableArena then
		for n = 1, 5 do
			spawnHelper(self, 'arena' .. n, 'TOPLEFT', player, 'BOTTOMRIGHT', 350, 510 - (56 * n))
		end
	end
	...
	CreateShadow(oUF_FreeArena1)
	CreateShadow(oUF_FreeArena2)
	CreateShadow(oUF_FreeArena3)
	CreateShadow(oUF_FreeArena4)
	CreateShadow(oUF_FreeArena5)
