...
FreeUIStatsButton:SetScript("OnUpdate", function(self, elapsed)
	...
	if last >= 1 then
		text:SetText("|cffffffff"..floor(GetFramerate() + .5).."|r fps   |cffffffff88|r/|cffffffff92|r ms   |cffffffff"..GameTime_GetTime(false))
		last = 0
	end
end)
...
FreeUIStatsButton:HookScript("OnEnter", function()
	...
	if C.bags.enable and BagsHolder:IsShown() then
		...
		GameTooltip:SetPoint("TOPRIGHT", Minimap, "TOPLEFT", -6, 0)
	end
